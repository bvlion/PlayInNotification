...
- AI agentは、作業中のIssueに対応する専用worktree内だけを変更してください。
- Codex Remote等で作業開始時点から専用worktreeが用意されている場合は、そのworktreeをそのまま使用し、`scripts/create-agent-worktree.sh`を追加実行しないでください。
- 手元の既存worktreeからIssue用worktreeを新しく作成する場合は、`scripts/create-agent-worktree.sh <agent> <short-task-name>`を実行してください。
- 既にAI agentを起動している場合は、現在のworktreeから同スクリプトで次のIssue用worktreeを作成し、そのworktreeを作業対象として続行して構いません。
- 新しいIssue用worktreeを作成した後は、そのworktreeだけを変更し、作成元worktreeや他のworktreeを変更しないでください。
...
- `local.properties`、署名情報、APIキー、アクセストークン等のローカル固有設定や秘密情報をGit管理対象にしたり、commitしたり、log出力したりしないでください。
- Git管理対象にしない範囲で、`local.properties`はローカルのworktree間でコピーして構いません。
