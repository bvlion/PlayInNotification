...
- 1プレイは30秒とする。
- 回答方式は3択のみとし、数字入力や文字入力を前提にしない。
- 難易度はゲームジャンルごとに5段階とし、ユーザーがアプリ本体で明示的に設定する。
- 正答率、連続正解、成長レベル等を理由に難易度を自動変更しない。
- 問題の難易度と、継続・回答によって上がる成長レベルは別の概念として扱う。
- 成績ではStreak、自己ベスト、累計回答数、プレイ回数、成長レベルを扱う。
- 累計正解数や正答率を成績として見せることを目的にしない。
- アプリ本体では、失敗を評価するより継続や積み上げを肯定的に見せることを優先する。
- 正誤フィードバックの表示時間やメイン画面の具体的な見せ方など、操作感に依存する内容は実機利用後に調整する。
- 現時点のMVPでは、他アプリや動画広告の検知・操作を要件に含めない。

確定済みの詳細仕様はGitHub Issueを正とし、実装都合だけを理由に変更しないでください。
...
- 確定済みの仕様や外部契約を、実装都合だけを理由に変更しないでください。
- 将来必要になるかもしれない機能や抽象化を、現在のIssueに必要でない段階で先回りして追加しないでください。
...
- 依存関係をversion catalogで管理する構成になった場合は、既存の管理方法を維持してください。

### コメント

- コメントやKDocは必要な場合に限って追加してください。
- コードだけでは残せない理由、制約、意図が将来の変更判断に必要な場合に記述してください。
- クラス名、関数名、変数名や直後の処理を言い換えるだけのコメントやKDocは書かないでください。
- 記述する場合は日本語としてください。
...
- Androidのproductionコード、テスト、build設定等へ影響する変更では、変更範囲に対応する最小限かつ十分なunit test、lint、buildを実行してください。
...
- UI確認を目的としたinstrumented test、emulator操作、adb操作を代替手段として勝手に追加しないでください。
...
- Issue対応を依頼された場合は、調査、実装、検証、commit、pushを行い、Pull RequestをDraftではなくReady for reviewとして作成してください。
- Pull Request本文には対応Issue、目的または原因、変更内容、最終的な検証結果を記載してください。
- 影響のある未実施または失敗した検証が残る場合はPull Request本文に記載してください。
- UI変更で実機確認が有用な場合は、AIエージェント自身では操作せず、ユーザーが確認すべき項目をPull Request本文または完了報告に記載してください。
